...
// Replaced framer-motion with pure CSS transitions for drawer/overlay animations
                        ...
                        items.map((item) => (
                            <div key={`${item.variantId}-${item.size}`} className="flex gap-4 group">
                                <div className="w-24 h-24 bg-gray-50 rounded-2xl flex items-center justify-center p-2 border border-gray-100 shrink-0">
                                    <img src={item.image} alt={item.name} className="w-full h-auto mix-blend-multiply" width="80" height="80" loading="lazy" />
                                </div>
                                <div className="flex-1 flex flex-col justify-between py-1">
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <div className="flex items-center border border-gray-200 rounded-lg">
                                            <button onClick={() => handleUpdateQuantity(item.variantId, item.size, item.quantity - 1)} className="p-1.5 hover:bg-gray-50 text-gray-500">
